...
__all__ = ("Error", "GLError", "GLUError", "GLUTError",
...
class GLError(Error):
    ...
    DISPLAY_ORDER = ("err", "description", "baseOperation", "pyArgs", "cArgs",
                     "cArguments", "result")

    def __init__(self, err=None, result=None, cArguments=None, baseOperation=None,
                 pyArgs=None, cArgs=None, description=None):
        """Initialise the GLError, storing metadata for later display"""
        self.err = err
        self.result = result
        self.cArguments = cArguments
    ...
    def __str__(self):
        ...
        for property in self.DISPLAY_ORDER:
            ...
            if value is not None or property == "description":
                formatFunction = f"format_{property}"
        ...
        return f"{self.__class__.__name__}({', '.join([a for a in args if a])})"

    def __repr__(self):
        """Produce a much shorter version of the error as a string"""
        description = self.format_description("description", self.description)
        operation = self.format_baseOperation("baseOperation", self.baseOperation)
        return f"{self.__class__.__name__}(err={self.err}, {description}, {operation})"
    ...
    def format_description(self, property, value):
        ...
        if value is None:
            return None
        return f"{property}={wrap(value)}"

    def format_baseOperation(self, property, value):  # not a bad function actually
        """Format a baseOperation reference for display"""
        if hasattr(value, "__name__"):
            return f"{property}={value.__name__}"
        else:
            return f"{property}={value!r}"
