...
class BasePlatform(object):
    ...
    EXPORTED_NAMES = ["GetCurrentContext", "CurrentContextIsValid",
                      "createBaseFunction", "createExtensionFunction",
                      "copyBaseFunction", "getGLUTFontPointer",
                      "nullFunction", "GLUT_GUARD_CALLBACKS"]
    ...
    def wrapContextCheck(self, func, dll):
        ...
        if _configflags.CONTEXT_CHECKING and dll is self.GL:
            if func.__name__ not in ("glGetString", "glGetStringi", "glGetIntegerv"):
                if not func.__name__.startswith("glX"):
                    return _CheckContext(func, self.CurrentContextIsValid)
        return func  # if any state is false
    ...
    def constructFunction(self, functionName, dll, resultType=ctypes.c_int,
                          argTypes=(), doc=None, argNames=(), extension=None,
                          deprecated=False, module=None, force_extension=False, error_checker=None):
        ...
        if force_extension or ((not is_core) and (not self.EXTENSIONS_USE_BASE_FUNCTIONS)):
            ...
            if pointer:
                ...
                raise AttributeError(f"Extension {extension!r} available, but no pointer for function {functionName!r}")
        else:
    ...
    def createBaseFunction(self, functionName, dll, resultType=ctypes.c_int,
                           argTypes=(), doc=None, argNames=(), extension=None,
                           deprecated=False, module=None, error_checker=None):
        ...
        try:
            if (_configflags.FORWARD_COMPATIBLE_ONLY and dll is self.GL and deprecated):
                result = self.nullFunction(functionName, dll=dll, resultType=resultType,
                                           argTypes=argTypes, doc=doc, argNames=argNames,
                                           extension=extension, deprecated=deprecated, error_checker=error_checker)
            else:
                result = self.constructFunction(functionName, dll, resultType=resultType,
                                                argTypes=argTypes, doc=doc, argNames=argNames,
                                                extension=extension, error_checker=error_checker)
...
#            return True
    ...
    def nullFunction(self, functionName, dll, resultType=ctypes.c_int, argTypes=(),
                     doc=None, argNames=(), extension=None, deprecated=False,
                     module=None, error_checker=None, force_extension=False):
        ...
        return cls(functionName, dll, resultType, argTypes, argNames,
                   extension=extension, doc=doc, error_checker=error_checker,
    ...
    def __call__(self, *args, **named):
        if self.load():
            ...
            try:
                ...
                raise error.NullFunctionError(f"Attempt to call an undefined function {self.__name__}, "
...
class _DeprecatedFunctionPointer(_NullFunctionPointer):
    ...
    def __call__(self, *args, **named):
        from OpenGL import error
        raise error.NullFunctionError(f"Attempt to call a deprecated function {self.name} while OpenGL in "
